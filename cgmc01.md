...
Welcome to lesson 3.  We pick up this lesson by answering the last question in section 2.  In what way could the prompt asking chat GPT to discuss the principles of prompt engineering be improved?  So, in this lesson, demonstrate the method of iteration of improving the output of a prompt.  This is by using different prompts successively.  One could achieve this by using a different prompt altogether while another strategy would be by providing further context and yet another strategy would be by further narrowing down the initial prompt to ask chat GPT provide further details on the previous prompt response.  In the prompt below we use the first strategy by asking it to approach the question in a different way by providing good and bad examples.  Here is the prompt.
